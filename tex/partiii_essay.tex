...
\section{Examining Robustness Properties of the SDP}
\subsection{Approximating SDP of adjacency matrices / Looking at the unproved lemmas and extension to graphons}
We will now briefly sketch out the proofs of Lemmas \ref{lem:SDP.approx1} and \ref{lem:SDP.approx2}
by highlighting the main ideas (see the original paper \cite{MS16} for details).
We will then apply these ideas to show that the algorithm works for a more general class of random graph models, 
...
\end{definition}
\begin{remark}
Note that the proof of weak recovery in \cref{sec:weak.recovery} uses the adjacency matrix of the graph 
only in two places: the SDP approximation lemmas (Lemmas \ref{lem:SDP.approx1} and \ref{lem:SDP.approx2}),
...
Thus, to show that the algorithm still works for the perturbed SBM,
we only need to show that similar SDP approximation results still hold for the perturbed SBM.
\end{remark}
\subsubsection{General approach to approximating SDP of adjacency matrices}
\label{sec:bounds}
The main idea of approximating the SDP of adjacency matrices is to utilize the following two helper functions:
...
For $\beta > 0$, define $\Phi(\beta, k; \bm{M})$, the smoothed version of $\mathcal{OPT}_k(\bm{M})$, as follows:
\[
...
\]
\end{theorem}
\todo{briefly bound SDP to Phi for B(lambda) and cent adj matrix}
