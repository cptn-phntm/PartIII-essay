...
\subsubsection{Step 2: There exists a good label estimator among $\{\hat{\bm{x}}^{(i,j)}\}_{i,j \in [n]}$}
...
\subsubsection{Step 3: The evaluation edge set gives a good estimator}
...
& \leq |2\hat{\bm{x}}_a \hat{\bm{x}}_b| \leq 2.
\end{align*}
...
as desired.
\end{proof}
Now, since $|E_\mathrm{eval}| \sim \operatorname{Binomial}(\binom{n}{2}, n^{-1/2}/(1 + n^{-1/2}))$,
we have 
