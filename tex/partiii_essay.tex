...
\todo{explain}
\subsection{Approximation of the SDP of adjacency matrices with the SDP of deformed GOE matrices}
\label{sec:sdp.approx}
In this section, we briefly outline the main ingredients behind results like Lemmas \ref{lem:SDP.approx1} 
and \ref{lem:SDP.approx2}, stating the key theorems used in their proofs without going into the details;
we refer the reader to their original paper \cite{MS16} for complete arguments.

In the following section, we will show how these ingredients can be combined to prove weak recovery 
for a more general class of random graph models, demonstrating another layer of robustness.

\subsubsection{Helper functions}
\label{sec:sdp.helper}
\todo{check for sqrt d factor}
...
\end{theorem}
\begin{remark}
Clearly, $\bm{A}^\mathrm{cen}_{G}$ and $\bm{B}(\lambda)$ are symmetric matrices, so 
the above bound applies to them.
\todo{derive line 49 and 50}
\end{remark}
...
applies to $\bm{A}^\mathrm{cen}_{G}$ and $\bm{B}(\lambda)$.
\end{remark}
...
\begin{enumerate}
  ...
  then $\mathbb{E}(\|\bm{H}\|_\text{op}) \leq C d^{-1/2 - \alpha}$.
\end{enumerate}
...
Furthermore, note that all the theorems in \cref{sec:sdp.helper} for the centered adjacency matrix
only require the graph to have a bounded degree, so they will still hold for the perturbed SBM.
\end{remark}

Therefore, in this section, we will first demonstrate how the interpolation step works
works for the perturbed SBM, and then put everything together to get a similar SDP approximation 
result as in Lemmas \ref{lem:SDP.approx1} and \ref{lem:SDP.approx2}, which will prove theorem 
\ref{thm:perturbed.SBM}.
\subsubsection{Lindeberg Interpolation}
...
\subsubsection{Coupling of Gaussian Matrices}
...
\[
\mathbb{E} \| \bm{X} \| \leq (1+\epsilon) \left(2\sigma^*_\text{row} 
+ \frac{6}{\sqrt{\log(1+\epsilon)}} \sigma^*_\text{elt} \sqrt{\log n}\right)
\]
\end{theorem}
...
\leq (1+\epsilon) \left(2 + \frac{6}{\sqrt{\log(1+\epsilon)}} \sqrt{\frac{\log n}{n}} \right)\sqrt{\delta V}
\leq C \sqrt{\delta V},
\]
...
- \mathbb{E}F(\bm{B})\right| \leq \frac{C\beta^2}{\sqrt{d}} + Cd^{-\alpha}.
\]
\subsubsection{(tie all the ineqs together)}
\todo{tie all the ineqs together}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                             
% REFERENCES
%                                                                             
