...
$F(\bm{M}) := \mathbb{E}\Phi(\beta, k; \bm{M})$ with $\bm{X} = \bm{A}^\mathrm{cen}_{G} / \sqrt{d}$ and 
...
To bound the difference between $F(\bm{D})$ and $F(\bm{B}(\lambda))$, we first notice that, for all 
matrices $\bm{M}, \bm{M}' \in \mathbb{R}^{n \times n}$, we have 
\begin{align*}
\Phi(\beta, k; \bm{M}) &= \frac{1}{\beta} \log \left(\int_{(\mathbb{S}^{k-1})^n} \exp\left(\beta \sum_{i,j=1}^n M_{ij}
\langle \bm{\sigma}_i, \bm{\sigma}_j \rangle\right) d\nu(\bm{\sigma})\right)\\
&=
...
Hence,
\[
\left| \frac{1}{n}\Phi(\beta, k; \bm{M}) - \frac{1}{n}\Phi(\beta, k; \bm{M}')\right| 
\leq \|\bm{M} - \bm{M}'\|_{\mathrm{op}}.
\]
Therefore,
\begin{align*}
\frac{1}{n} \left|F(\bm{D}) - F(\bm{B}(\lambda))\right|
&= \frac{1}{n} \left|\mathbb{E}\Phi(\beta, k; \bm{D}) - \mathbb{E}\Phi(\beta, k; \bm{B}(\lambda))\right|\\
&\leq \frac{1}{n} \mathbb{E} \left|\Phi(\beta, k; \bm{D}) - \Phi(\beta, k; \bm{B}(\lambda))\right|\\
&\leq \mathbb{E} \|\bm{D} - \bm{B}(\lambda)\|_{\mathrm{op}}.
\end{align*}
Thus, we want to find a bound on $\mathbb{E} \|\bm{D} - \bm{B}(\lambda)\|_{\mathrm{op}}$. To do this,
...
\todo{check signs}
\[
\bm{B}(\lambda)_{ij} \sim \mathcal{N}\left(
\frac{\lambda}{n} x_i x_j, \ \frac{1}{n}\right).
\]
\todo{diagonal}
Thus, we can couple $\bm{D}$ and $\bm{B}(\lambda)$ such that
\[
...
\mathbb{E} \|\frac{\sqrt{d}}{n} \bm{H} + \bm{Z_D} - \bm{Z_B}\|_{\mathrm{op}}
